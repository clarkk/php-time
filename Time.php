...
class Time {
	...
	static public function time_offset_days(int $time, int $days, bool $count_start_day=false): int{
		if($count_start_day){
			if($days > 0){
				$days--;
			}
	...
	static public function time_offset_months(int $time, int $months, bool $count_start_day=false): int{
		$day 	= date('d', $time);
		$month 	= date('n', $time);
		$year 	= date('Y', $time);
